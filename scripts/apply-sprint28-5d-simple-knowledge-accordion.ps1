...
$cssPatchPath = Join-Path $repoRoot "patches\sprint28-5d-simple-knowledge-accordion.css"
$marker = "/* Sprint 28.5D: Simple Knowledge Accordion UX */"
...
if (!(Test-Path $cssPatchPath)) {
  throw "Cannot find patches\sprint28-5d-simple-knowledge-accordion.css. Make sure the patch zip was extracted first."
}
...
if ($globalsContent -notlike "*$marker*") {
  ...
  Write-Host "Sprint 28.5D CSS appended to apps\web\app\globals.css" -ForegroundColor Green
} else {
  Write-Host "Sprint 28.5D CSS already exists in apps\web\app\globals.css; skipping append." -ForegroundColor Yellow
}
...
Write-Host ""
Write-Host "Sprint 28.5D simple knowledge accordion patch is in place." -ForegroundColor Cyan
