...
$PatchPath = Join-Path $Root "patches\sprint28-6a2-chevron-css-triangle-fix.css"

if (!(Test-Path $GlobalsPath)) { throw "Cannot find apps\web\app\globals.css. Run this from the noor repo root." }
if (!(Test-Path $PatchPath)) { throw "Cannot find patches\sprint28-6a2-chevron-css-triangle-fix.css." }

$Start = "/* NOOR SPRINT 28.6A.2 HOTFIX - CHEVRON CSS TRIANGLE FIX START"
$End = "/* NOOR SPRINT 28.6A.2 HOTFIX - CHEVRON CSS TRIANGLE FIX END */"
...
if ($Globals -match $Pattern) {
  ...
  Write-Host "Replaced existing chevron CSS triangle fix block."
} else {
  Add-Content -Path $GlobalsPath -Value "`r`n`r`n$Patch" -Encoding UTF8
  Write-Host "Appended chevron CSS triangle fix block."
}
